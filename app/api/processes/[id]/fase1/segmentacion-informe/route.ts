...
import { buildSegmentacionInformeDocx } from "@/lib/segmentacion-informe";
  ...
  try {
    ...
    if (!hitos.A2) {
      return NextResponse.json(
        { error: "Guarda el paso A2 (Segmentación) antes de generar el informe." },
        { status: 409 },
      );
    }
    ...
    const buffer = await buildSegmentacionInformeDocx({
      entidad: proceso.entity ?? necesidad?.entidad ?? auth.user.entity ?? "Entidad contratante",
    ...
    const filename = `Informe-Segmentacion-${slugify(proceso.nomenclature || "expediente")}.docx`;
